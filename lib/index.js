 ...
 * 仅支持 DeepSeek 官方在售及兼容期内的模型（deepseek-flash / deepseek-v4-pro /
 * deepseek-v4-flash / deepseek-v4-flash-vision-exp）。价格数据来自
 * lib/pricing-data.json（由插件作者随官方调价维护），按每条 assistant 消息完成时刻取价：
 * 峰谷定价（2026-08-17 起）高峰为北京时间**周一至周五** 09:00–12:00 / 14:00–18:00，
 * 其余时段（含周末）空闲半价；旧模型名的兼容路由（2026-09-10 起 v4-flash 系列、
 * 2026-09-14 12:00 起 v4-pro 全部路由至 V4.1 Flash）声明在数据文件的
 * `policies[].routes`，按真正提供服务的模型单价计费。
 * 非上述模型的会话不显示第二行。
 *
 * 端点（仅回环）：
...
class SessionCostLedger {
  ...
  /**
   * 标记某会话含不支持的模型（用于隐藏第二行），并记下具体的模型名——官方改名/扩表后
...
function recordSessionEvent(ledger, headersBySession, session, event, pricing) {
  ...
  // 先判断模型，再判断 usage：usage 是可选字段，但不支持的模型仍应隐藏第二行。
...
export { Config, SessionCostLedger, apply, inject, name, recordSessionEvent };
