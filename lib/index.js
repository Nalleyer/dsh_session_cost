 ...
 * 仅支持 DeepSeek 官方三个模型（deepseek-v4-flash / deepseek-v4-pro /
 * deepseek-v4-flash-vision-exp）。价格数据来自
...
class SessionCostLedger {
   ...
   * 被裁剪掉的旧消息（trimmed 聚合）与 unsupported 会话不参与逐条重建，直接保留。
   */
  reprice(pricing) {
    ...
    for (const extra of extras) {
      ...
      if (extra.unsupported === true) session.unsupported = true;
