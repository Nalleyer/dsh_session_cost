...
const { timezone, peakWindows, policies } = DATA;
...
function makePricing(hash) {
  return {
    hash,
    at: (model, time) => priceAt(model, time, { timezone, peakWindows, policies })
  };
}
...
test("重估保留 unsupported 会话（supported=false 不被重置）", () => {
  const ledger = makeLedger(10);
  ledger.markUnsupported("s9");
